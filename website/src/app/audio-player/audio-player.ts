...
function buildSpectrogram(
  ...
  for (let f = 0; f < frames; f++) {
    ...
    for (let py = 0; py < canvasH; py++) {
      const freq = Math.pow(
        2,
        minF + ((canvasH - 1 - py) / (canvasH - 1)) * (maxF - minF),
      );
...
export class AudioPlayerComponent implements OnChanges, OnDestroy {
  ...
  async ngOnChanges() {
    ...
    const normalized = this.wavBase64
      .replace(/[\s]/g, "")
      .replace(/-/g, "+")
      .replace(/_/g, "/");
    const arrayBuffer = await fetch(`data:audio/wav;base64,${normalized}`).then(
      (r) => r.arrayBuffer(),
    );
    ...
    setTimeout(() => this.drawSpectrogram(), 0);
  }
  ...
  private tick() {
    this.rafId = requestAnimationFrame(() => {
      ...
      this.zone.run(() => this.currentTime.set(t));
      this.tick();
    });
  }
