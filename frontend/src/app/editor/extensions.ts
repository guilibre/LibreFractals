...
import { lineColToOffset, offsetToLineCol, showHoverTooltip } from './lsp-utils';
...
export function completionsExtension(fractal: FractalService) {
  const source = async (ctx: CompletionContext) => {
    ...
    if (!raw) return null;
    const items = JSON.parse(raw) as Array<{ label: string; detail: string; kind: string }>;
    const options: Completion[] = items.map((item) => ({
      label: item.label,
      detail: item.detail,
      type: item.kind === 'keyword' ? 'keyword' : item.kind === 'builtin' ? 'function' : 'variable',
    }));
...
export function hoverExtension(fractal: FractalService) {
  return EditorView.domEventHandlers({
    mousemove: (event, view) => {
      ...
      void fractal.getHover(source, line, col).then((raw) => {
